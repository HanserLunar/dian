...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// /*此块与读取mpu6050无关，仅属于学习产物（草稿）//////////// i2c_cmd_handle_t cmd_struct; uint8_t data[128]; static esp_err_t readMessage() { int ret; cmd_struct=i2c_cmd_link_create();//创建一个i2c命令链接，为之后的i2c操作执行 i2c_master_start(cmd_struct);//i2c运行开始函数，只是开始标记，初始化cmd_struct

//发送一个字节数据 包括从机地址 读写方向 0表示主机输出 1表示从机输出 i2c_master_write_byte(cmd_struct,mm<<1|I2C_MASTER_READ,0X1);//存疑//向mpu6050读取数据，等待从机返回数据

//再发一个字节数据，包括从机寄存器地址 i2c_master_write_byte(cmd_struct,0x3B,0x1);

//重新发起一次通信，改变通信方向 i2c_master_start(cmd_struct);

//从该寄存器读取一个字节数据,存入data数组 i2c_master_read_byte(cmd_struct,data,0x0);

//无需应答 //停止通信 i2c_master_stop(cmd_struct); ret=i2c_master_cmd_begin(I2C_NUM_0,cmd_struct,1000/portTICK_PERIOD_MS);

//i2c_cmd_link_delete(cmd_struct); //i2c_master_read() return ret; } */ ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
